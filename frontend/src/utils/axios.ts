...
import { getCompanyId, STORAGE_KEY_CONSTANT } from "./constants";
...
instance.interceptors.response.use(
  ...
  (error) => {
    if (
      error.response.data?.message === "User is Inactive" ||
      error.response?.statusText === "User is Inactive"
    ) {
      localStorage.clear();
